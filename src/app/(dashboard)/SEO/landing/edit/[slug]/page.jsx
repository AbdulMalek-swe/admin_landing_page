...
const SEOCreateForm = ({ params }) => {
  const [seoSingleData, setSeoSingleData] = React.useState(null); 
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      title:  seoSingleData?.title||"",
    ...
    onSubmit: async (values) => {
      ...
      console.log(values);
      try {
        const response = await privateRequest.post(`seo/${params?.slug}`, formData);
  ...
  const fetchBlog = useCallback(async () => {
    try {
      const response = await privateRequest.get(`seo/${params?.slug}`);
      if (responseCheck(response)) {
        setSeoSingleData(response.data?.data);
      }
    } catch (error) {
      console.log(error);
      errorHandler(error);
